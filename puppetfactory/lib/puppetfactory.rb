...
class Puppetfactory  < Sinatra::Base
    ...
    helpers do
      ...
      def skeleton(username)
        ...
        `docker run --add-host "master.puppetlabs.vm puppet:172.17.42.1" --name="#{username}" -p #{port}:80 -h #{username}.#{USERSUFFIX} -e RUNLEVEL=3 -d -v #{ENVIRONMENTS}/#{username}:/root/puppetcode -v /home/#{username}/share:/share #{CONTAINER_NAME} /sbin/init`
